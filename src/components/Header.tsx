...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 w-full items-center justify-between border-b border-white/10 bg-[#0A0A0B]/90 px-4 backdrop-blur-md lg:px-8">
      ...
      {/* Right Controls: Autonomous Engine Status, Quality Index, Run Action, Profile */}
      <div className="flex items-center gap-4">
        {/* Production Engine Status Badge */}
        <div 
          className="hidden sm:flex items-center gap-2 rounded-sm border border-emerald-500/20 bg-emerald-500/5 px-3 py-1.5 text-xs text-emerald-300"
          title="Autonomous Multi-Model Synthesis Engine Active"
        >
          <div className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse shadow-[0_0_6px_rgba(16,185,129,0.8)]" />
          <span className="font-mono text-[10px] uppercase tracking-wider font-semibold">
            Engine: Live Autonomous
          </span>
        </div>
